...
# Constants
HURIER_ELEMENTS = ["Hear", "Understand",
...
def create_scenario(industry):
    ...
    try:
        ...
        st.write("Scenario created:", scenario)  # Debug statement
        return scenario
...
def clean_up_scenario(scenario):
    ...
    try:
        ...
        st.write("Clean scenario created:", clean_scenario)  # Debug statement
...
def conversation_engine(character, context):
    ...
    try:
        ...
        st.write("Initial message generated successfully:",
                 initial_message)  # Debug statement
...
def continue_conversation(messages, user_message):
    try:
        ...
        st.write("Assistant response:", assistant_response)  # Debug statement

        return assistant_response, messages

    except Exception as e:
        st.error(
            f"An error occurred while continuing the conversation: {str(e)}")
...
def analyze_response(element, user_response, assistant_message):
    ...
    try:
        ...
        st.write("Feedback generated:", feedback)  # Debug statement
        return feedback
    except Exception as e:
...
def listening_skill_coach(assistant_message):
    ...
    for element in HURIER_ELEMENTS:
        ...
        user_response = st.text_input(
            f"Your answer for {element}:", key=f"input_{element}")

        if st.button(f"Submit {element}", key=f"submit_{element}"):
            feedback = analyze_response(
                element, user_response, assistant_message)
...
def main():
    ...
    # Industry selection
    industry = st.selectbox("Select an industry:", [
    ...
    if st.button("Generate Scenario"):
        ...
        if scenario:
            ...
            if clean_scenario:
                ...
                # Reset conversation when new scenario is generated
        ...
        if 'conversation' not in st.session_state:
            ...
            with st.spinner('Please wait while the conversation is being initialized...'):
                try:
                    ...
                    st.error(
                        f"An error occurred while initializing the conversation: {str(e)}")

        if st.session_state.conversation:
            st.subheader("Conversation:")
            st.write("Character:",
                     st.session_state.conversation['initial_message'])
            ...
            if st.button("Submit Response"):
                ...
                try:
                    ...
                    st.error(
                        f"An error occurred during the conversation: {str(e)}")
        else:
